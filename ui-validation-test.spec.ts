...
test.describe('UI 중복 요소 검증', () => {
  test('골프장 예약 관리 시스템 UI 중복 검증', async () => {
    ...
    for (const text of duplicateTexts) {
      ...
      if (elements.length > 1) {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          const parentElement = await elements[i].evaluateHandle(el => {
    ...
    if (await hamburgerMenu.isVisible()) {
      ...
      const sidebarDuplicates = sidebarTexts.filter((item, index) => 
    ...
    if (await userProfile.isVisible()) {
      ...
      const dropdownDuplicates = dropdownTexts.filter((item, index) => 
    ...
    const totalDuplicates = textDuplicates.length + 
                            (sidebarDuplicates?.length || 0) + 
                            (dropdownDuplicates?.length || 0) + 
                            (navDuplicates?.length || 0);
